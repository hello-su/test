function [ AdjustmentCostTotalPer,DiffNumberPer,cfinal,ufinal,ffinal,vfinal,FVAL ] = InvCostAdjUFL(m,n,dataread)
...
fobj = [w;zeros(2*m+3*mn,1)];
%f = zeros(4*m+5*mn,1);

A = [zeros(1,3*m+3*mn),-ones(1,m),zeros(1,2*mn)];
B = -ObjUFL;

Aeq1 = zeros(m,4*m+5*mn);
Beq1 = zeros(m,1);
for i=1:m
    Aeq1(i,2*m+2*mn+i)=-1;
    for j=1:mn
...
Aeq3 = [zeros(1,2*m+2*mn),v0',u0',zeros(1,m+2*mn)];
Beq3 = ObjUFL;

Aeq4 = zeros(m,4*m+5*mn);
Beq4 = -f;
for i=1:m
    Aeq4(i,i)=1;
    Aeq4(i,m+i)=-1;
...
LB = [zeros(2*m+2*mn,1);-inf*ones(m+mn,1);zeros(m+2*mn,1)];
UB = inf*ones(4*m+5*mn,1);
